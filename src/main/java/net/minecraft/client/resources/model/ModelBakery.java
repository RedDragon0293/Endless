...
public class ModelBakery {
    ...
    private Set<ResourceLocation> getVariantsTextureLocations() {
        ...
        Collections.sort(list, Comparator.comparing(ModelResourceLocation::toString));
    ...
    public static void fixModelLocations(ModelBlock modelBlock, String path) {
        ResourceLocation resourcelocation = fixModelLocation(modelBlock.getParentLocation(), path);

        if (resourcelocation != modelBlock.getParentLocation()) {
            modelBlock.parentLocation = resourcelocation;
        }

        Map<String, String> map = modelBlock.textures;

        if (map != null) {
            for (Entry<String, String> entry : map.entrySet()) {
                String s = entry.getValue();
                String s1 = fixResourcePath(s, path);
