...
public class GuiOptions extends GuiScreen implements GuiYesNoCallback {
    ...
    public void initGui() {
        ...
        for (GameSettings.Options option : field_146440_f) {
            if (option.getEnumFloat()) {
                this.buttonList.add(new GuiOptionSlider(option.returnEnumOrdinal(), this.width / 2 - 155 + i % 2 * 160, this.height / 6 - 12 + 24 * (i >> 1), option));
            } else {
                GuiOptionButton guioptionbutton = new GuiOptionButton(option.returnEnumOrdinal(), this.width / 2 - 155 + i % 2 * 160, this.height / 6 - 12 + 24 * (i >> 1), option, this.game_settings_1.getKeyBinding(option));
