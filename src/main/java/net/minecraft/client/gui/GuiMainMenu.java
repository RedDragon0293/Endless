...
public class GuiMainMenu extends GuiScreen implements GuiYesNoCallback {
    ...
    public void confirmClicked(boolean result, int id) {
        if (result && id == 12) {
            ...
            if (result) {
                try {
                    Desktop.getDesktop().browse(new URI(this.openGLWarningLink));
    ...
    private void rotateAndBlurSkybox(float p_73968_1_) {
        ...
        for (int index = 0; index < blurLevel; ++index) {
            float alpha = 1.0F / (float) (index + 1);
            int width = this.width;
            int height = this.height;
            float offset = (float) (index - i / 2) / 256.0F;
            worldrenderer.pos(width, height, this.zLevel).tex(0.0F + offset, 1.0D).color(1.0F, 1.0F, 1.0F, alpha).endVertex();
            worldrenderer.pos(width, 0.0D, this.zLevel).tex(1.0F + offset, 1.0D).color(1.0F, 1.0F, 1.0F, alpha).endVertex();
            worldrenderer.pos(0.0D, 0.0D, this.zLevel).tex(1.0F + offset, 0.0D).color(1.0F, 1.0F, 1.0F, alpha).endVertex();
            worldrenderer.pos(0.0D, height, this.zLevel).tex(0.0F + offset, 0.0D).color(1.0F, 1.0F, 1.0F, alpha).endVertex();
        }
