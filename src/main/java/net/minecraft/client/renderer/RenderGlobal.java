...
public class RenderGlobal implements IWorldAccess, IResourceManagerReloadListener {
    ...
    private LongHashMap<Chunk> worldChunkProviderMap = null;
    ...
    public void renderEntities(Entity renderViewEntity, ICamera camera, float partialTicks) {
        ...
        if (this.renderEntitiesStartupCounter > 0) {
            ...
            for (ContainerLocalRenderInformation e : this.renderInfosEntities) {
                Chunk chunk = e.renderChunk.getChunk();
                ClassInheritanceMultiMap<Entity> classinheritancemultimap = chunk.getEntityLists()[e.renderChunk.getPosition().getY() / 16];
            ...
            for (ContainerLocalRenderInformation e : this.renderInfosTileEntities) {
                List<TileEntity> list1 = e.renderChunk.getCompiledChunk().getTileEntities();
    ...
    public void setupTerrain(Entity viewEntity, double partialTicks, ICamera camera, int frameCount, boolean playerSpectator) {
        ...
        if (!flag && this.displayListEntitiesDirty && !Shaders.isShadowPass) {
            ...
            while (!deque.isEmpty()) {
                RenderGlobal.ContainerLocalRenderInformation info = deque.poll();
                RenderChunk renderchunk6 = info.renderChunk;
                EnumFacing enumfacing1 = info.facing;
        ...
        if (Shaders.isShadowPass) {
            ...
            for (RenderGlobal.ContainerLocalRenderInformation info : this.renderInfos) {
                RenderChunk chunk = info.renderChunk;

                if (chunk.isNeedsUpdate() || set.contains(chunk)) {
                    this.displayListEntitiesDirty = true;
                    BlockPos blockpos1 = chunk.getPosition();
                    boolean flag4 = blockpos.distanceSq(blockpos1.getX() + 8, blockpos1.getY() + 8, blockpos1.getZ() + 8) < 768.0D;

                    if (!flag4) {
                        this.chunksToUpdate.add(chunk);
                    } else if (!chunk.isPlayerUpdate()) {
                        this.chunksToUpdateForced.add(chunk);
                    } else {
            ...
            if (d0 * d0 + d1 * d1 + d2 * d2 > 1.0D) {
                ...
                for (RenderGlobal.ContainerLocalRenderInformation info : this.renderInfos) {
                    if (info.renderChunk.compiledChunk.isLayerStarted(blockLayerIn) && k++ < 15) {
                        this.chunksToResortTransparency.add(info.renderChunk);
                    }
                }
            }
    ...
    private EntityFX spawnEntityFX(int particleType, boolean ignoreRange, double p_174974_3_, double p_174974_5_, double p_174974_7_, double p_174974_9_, double p_174974_11_, double p_174974_13_, int... p_174974_15_) {
        if (this.mc != null && this.mc.getRenderViewEntity() != null && this.mc.effectRenderer != null) {
            ...
            if (particleType == EnumParticleTypes.EXPLOSION_HUGE.getParticleID() && !Config.isAnimatedExplosion()) {
                return null;
            } else if (particleType == EnumParticleTypes.EXPLOSION_LARGE.getParticleID() && !Config.isAnimatedExplosion()) {
                return null;
            } else if (particleType == EnumParticleTypes.EXPLOSION_NORMAL.getParticleID() && !Config.isAnimatedExplosion()) {
                return null;
            } else if (particleType == EnumParticleTypes.SUSPENDED.getParticleID() && !Config.isWaterParticles()) {
                return null;
            } else if (particleType == EnumParticleTypes.SUSPENDED_DEPTH.getParticleID() && !Config.isVoidParticles()) {
                return null;
            } else if (particleType == EnumParticleTypes.SMOKE_NORMAL.getParticleID() && !Config.isAnimatedSmoke()) {
                return null;
            } else if (particleType == EnumParticleTypes.SMOKE_LARGE.getParticleID() && !Config.isAnimatedSmoke()) {
                return null;
            } else if (particleType == EnumParticleTypes.SPELL_MOB.getParticleID() && !Config.isPotionParticles()) {
                return null;
            } else if (particleType == EnumParticleTypes.SPELL_MOB_AMBIENT.getParticleID() && !Config.isPotionParticles()) {
                return null;
            } else if (particleType == EnumParticleTypes.SPELL.getParticleID() && !Config.isPotionParticles()) {
                return null;
            } else if (particleType == EnumParticleTypes.SPELL_INSTANT.getParticleID() && !Config.isPotionParticles()) {
                return null;
            } else if (particleType == EnumParticleTypes.SPELL_WITCH.getParticleID() && !Config.isPotionParticles()) {
                return null;
            } else if (particleType == EnumParticleTypes.PORTAL.getParticleID() && !Config.isPortalParticles()) {
                return null;
            } else if (particleType == EnumParticleTypes.FLAME.getParticleID() && !Config.isAnimatedFlame()) {
                return null;
            } else if (particleType == EnumParticleTypes.REDSTONE.getParticleID() && !Config.isAnimatedRedstone()) {
                return null;
            } else if (particleType == EnumParticleTypes.DRIP_WATER.getParticleID() && !Config.isDrippingWaterLava()) {
                return null;
            } else if (particleType == EnumParticleTypes.DRIP_LAVA.getParticleID() && !Config.isDrippingWaterLava()) {
                return null;
            } else if (particleType == EnumParticleTypes.FIREWORKS_SPARK.getParticleID() && !Config.isFireworkParticles()) {
                return null;
            } else {
                if (!ignoreRange) {
                    double d3 = 1024.0D;

                    if (particleType == EnumParticleTypes.CRIT.getParticleID()) {
                        d3 = 38416.0D;
                    }
                ...
                EntityFX entityfx = this.mc.effectRenderer.spawnEffectParticle(particleType, p_174974_3_, p_174974_5_, p_174974_7_, p_174974_9_, p_174974_11_, p_174974_13_, p_174974_15_);

                if (particleType == EnumParticleTypes.WATER_BUBBLE.getParticleID()) {
                    CustomColors.updateWaterFX(entityfx, this.theWorld, p_174974_3_, p_174974_5_, p_174974_7_, this.renderEnv);
                }

                if (particleType == EnumParticleTypes.WATER_SPLASH.getParticleID()) {
                    CustomColors.updateWaterFX(entityfx, this.theWorld, p_174974_3_, p_174974_5_, p_174974_7_, this.renderEnv);
                }

                if (particleType == EnumParticleTypes.WATER_DROP.getParticleID()) {
                    CustomColors.updateWaterFX(entityfx, this.theWorld, p_174974_3_, p_174974_5_, p_174974_7_, this.renderEnv);
                }

                if (particleType == EnumParticleTypes.TOWN_AURA.getParticleID()) {
                    CustomColors.updateMyceliumFX(entityfx);
                }

                if (particleType == EnumParticleTypes.PORTAL.getParticleID()) {
                    CustomColors.updatePortalFX(entityfx);
                }

                if (particleType == EnumParticleTypes.REDSTONE.getParticleID()) {
